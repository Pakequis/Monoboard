...
| `wifi_manager.h/.cpp` | `wifiConnect()` / `wifiDisconnect()`, plus `isWifiConnected()` for the header's status line, persisting the last connection result across deep sleep (`RTC_DATA_ATTR`) so it stays accurate on wake cycles that skip WiFi entirely. |
...
|-----------|---------|---------------------------------|--------------|
| `x`       | int16_t | X coordinate (column)           | N/A          |
| `y`       | int16_t | Y coordinate (baseline)         | N/A          |
| `text`    | string  | Text to display                 | N/A          |
