...
// before the first readLocalSensors() call. The AS3935 only gets its full
// register configuration on a cold boot -- an ordinary timer wake just
// brings the I2C bus back up, since the chip keeps its config across the
// ESP32's deep sleep.
