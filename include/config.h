...
// The AS3935's distance-table sentinel for "storm out of range / distance
// unknown" (datasheet register value 0x3F = 63), returned as-is by
// distanceToStorm() -- not a real 63 km reading. Rejected in
// onConfirmedLightningStrike() before it can be recorded or drawn as a
// strike ring.
...
#define LIGHTNING_UNIT_LABEL_RIGHT_INSET 4  // px, right inset from the box border for the "km" unit label
#define LIGHTNING_UNIT_LABEL_Y_OFFSET   13  // px, baseline y offset below the rings area's top (i.e. below
                                         // the header bar), matching CALENDAR_WEEKDAY_LABEL_Y_OFFSET's
                                         // pitch for the same font just under a bar
