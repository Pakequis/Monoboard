  ...
  if (WiFi.status() != WL_CONNECTED)
  {
    ...
    if (haveLastBSSID)
    {
      ...
      if (!waitForConnection(WIFI_FAST_RECONNECT_TIMEOUT_MS))
      {
        ...
        // Clamped rather than a bare subtraction: both operands are
        // unsigned, so a future retune where the fast-reconnect timeout
        // reaches or exceeds the full timeout would otherwise underflow
        // to a multi-billion-ms wait instead of failing over quickly.
        unsigned long remainingTimeoutMs = (WIFI_TIMEOUT_MS > WIFI_FAST_RECONNECT_TIMEOUT_MS)
                                          ? (WIFI_TIMEOUT_MS - WIFI_FAST_RECONNECT_TIMEOUT_MS)
                                          : 0;
        waitForConnection(remainingTimeoutMs);
      }
    }
    else
    {
