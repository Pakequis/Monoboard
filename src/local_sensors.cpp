...
void initLocalSensors()
{
  ...
  // Full AS3935 (re)configuration only on a cold boot: the chip keeps its
  // config across the ESP32's deep sleep (it is never powered down), so an
  // ordinary timer wake only needs the bus brought back up to confirm the
  // sensor is still present, not every register rewritten again.
  bool coldBoot = esp_sleep_get_wakeup_cause() == ESP_SLEEP_WAKEUP_UNDEFINED;
  sensorPresentCache = coldBoot ? initAs3935() : beginAs3935Bus();
}
...
{
  // The AS3935 reports "storm out of range / distance unknown" as a real
  // distance-table value (63) rather than a separate flag -- discard it
  // instead of recording/drawing a strike at the ruler's outer edge as if
  // it were a genuine 63 km reading.
  if (km == AS3935_DISTANCE_OUT_OF_RANGE)
  {
    DEBUG_PRINTLN("Lightning strike rejected (distance out of range/unknown)");
    return;
  }
