...
// Shared "negativo" box header: a black bar across the box's top, with a
// white label centered in it. Used by both the lightning box's status bar
// and the calendar's month/year bar -- same visual, different heights/
// label baselines per caller.
static void drawBoxHeaderBar(int16_t boxX0, int16_t boxY0, int16_t boxWidth, int16_t headerHeight,
                              int16_t labelBaselineYOffset, const char* label)
{
  fillRect(boxX0, boxY0, boxWidth, headerHeight, GxEPD_BLACK);
  writeTextCenteredInBox(boxX0, boxWidth, boxY0 + labelBaselineYOffset, label, &FreeMonoBold9pt7b, GxEPD_WHITE);
}
...
void drawDashboard()
{
  updateScreen([]{
    ...
    drawBoxHeaderBar(lightningX0, lightningY0, lightningWidth, LIGHTNING_HEADER_HEIGHT,
                      LIGHTNING_HEADER_LABEL_Y_OFFSET, lightningLabel);
    ...
    // Fetched once here and reused below, instead of walking the history
    // ring buffer a second time with an identical loop just to draw it.
    uint8_t strikeCount = getStrikeHistoryCount();
    uint8_t strikeKmCache[STRIKE_HISTORY_COUNT];
    uint32_t strikeEnergyCache[STRIKE_HISTORY_COUNT];
    bool lightningCloseScale = (strikeCount > 0);
    for (uint8_t i = 0; i < strikeCount; i++)
    {
      getStrikeHistoryEntry(i, &strikeKmCache[i], &strikeEnergyCache[i]);
      if (strikeKmCache[i] > LIGHTNING_CLOSE_KM_THRESHOLD)
      {
    ...
    // stay bare numbers) and tucked in the box's top-right corner
    // instead, clear of both the rings (bounded to lightningMaxR, inscribed
    // well short of the corners) and the header bar below it.
    int16_t kmUnitLabelWidth = getTextWidth("km", &FreeMonoBold9pt7b);
    writeText(lightningX1 - LIGHTNING_UNIT_LABEL_RIGHT_INSET - kmUnitLabelWidth, lightningRingsTop + LIGHTNING_UNIT_LABEL_Y_OFFSET,
              "km", &FreeMonoBold9pt7b, GxEPD_BLACK);

    for (uint8_t i = 0; i < strikeCount; i++)
    {
      uint8_t strikeKm = strikeKmCache[i];
      uint32_t strikeEnergy = strikeEnergyCache[i];
    ...
    if (daysInMonth > 0)
    {
      ...
      drawBoxHeaderBar(calX0, calY0, CALENDAR_WIDTH, CALENDAR_HEADER_HEIGHT,
                        CALENDAR_TITLE_LABEL_Y_OFFSET, calTitle);
      ...
      for (int col = 0; col < 7; col++)
      {
        ...
        writeTextCenteredInBox(colX, (int16_t)calColWidth, weekdayRowTop + CALENDAR_WEEKDAY_LABEL_Y_OFFSET,
      ...
      for (int dayNum = 1; dayNum <= daysInMonth; dayNum++)
      {
        ...
        // writeTextCenteredInBox() cancels the glyph's own left-side
        // bearing, unlike a plain cursor-based centering -- no per-column
        // nudge needed even for the last column, flush against the box's
        // own right border with no grid line to absorb the drift.
        writeTextCenteredInBox(colX, colW, rowTop + CALENDAR_DAY_LABEL_Y_OFFSET, dayBuf, &FreeMonoBold9pt7b, dayColor);
      }
    }
    ...
    for (size_t hour = 0; hour < WEATHER_FORECAST_HOURS; hour++)
    {
      ...
      writeTextCenteredInBox(boxX, WEATHER_BOX_COLUMN_WIDTH, boxY + WEATHER_HOUR_LABEL_Y_OFFSET,
      ...
      writeTextCenteredInBox(boxX, WEATHER_BOX_COLUMN_WIDTH, boxY + WEATHER_ICON_Y_OFFSET,
