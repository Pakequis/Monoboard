...
// masking). Call only from a cold boot -- the chip keeps this
// configuration across the ESP32's deep sleep, so an ordinary timer wake
// only needs beginAs3935Bus(), and an IRQ wake must never call this,
// where writing REG0x03 would consume an unread strike interrupt.
// Returns true if the sensor is present.
bool initAs3935();
