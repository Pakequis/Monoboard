...
private:
  ...
  void feed(char c)
  {
    ...
    // Snapshot before the append below, so a fresh match attempt on this
    // character can record how long the title was *before* it -- needed to
    // trim exactly what was actually appended for a matched closing tag,
    // even if the RAW_TITLE_CAP guard stopped appending partway through it.
    size_t preAppendTitleLen = titleLen_;
    ...
    while (true)
    {
      if (c == pattern[matchPos_])
      {
        if (matchPos_ == 0)
        {
          matchStartTitleLen_ = preAppendTitleLen;
        }
        matchPos_++;
        break;
      }
    ...
    switch (state_)
    {
      ...
      case IN_TITLE:
      {
        ...
        // Trimming titleLen_ down to its value at the start of this match
        // (rather than always the full patternLen) keeps this correct even
        // when RAW_TITLE_CAP stopped the append partway through the tag, in
        // which case fewer than patternLen bytes of it ever landed in
        // rawTitle_ to begin with.
        titleLen_ = matchStartTitleLen_;
